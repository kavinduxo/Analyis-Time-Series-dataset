...
```{r}
#Neural Network
nn <- neuralnet(outputvar4[1][1]$~ inputvar4[1][1] + inputvar4[1][2] + inputvar4[1][3] + inputvar4[1][4], data=train_df, hidden=c(2,1), linear.output=FALSE, threshold=0.01)
nn$result.matrix
plot(nn)
```
...
```
```{r}
#find 11hour values for start and end
ts_df[1][1]
ts_df[430][1]
```
```{r}
...
tsdisplay(ts_df) # autoregression?
```
...
ggtsdisplay(ts_df,
            plot.type = "partial",
            main = "ACF & PACF plot for `ts_df' Time-Series",
            smooth = TRUE) 
```
```{r}
auto.arima(ts_df)
...
```{r}
#training dataset as time-series dataset
ts_all = ts(train_df, frequency = 1)
plot(ts_all) 
```
```{r}
# Checking the class
...
```{r}
par(mar = c(0.01,0.01,0.01,0.01))
tsdisplay(ts_all) # autoregression?
```
...
```{r}
#fitting a line
abline(reg=lm(trainSet~time(trainSet)))
```
...
```{r}
#time series
ts.plot(train_df[]$ElevenHr, xlab="Date", ylab="Power Consumption", main="Power Consumption 1/1/2018 - 5/22/2018")
```
...
```{r}
#Neural Network
nn <- neuralnet(train_df$NineHr ~ TenHr + ElevenHr, data=train_df, hidden=c(2,1), linear.output=FALSE, threshold=0.01)
nn$result.matrix
plot(nn)
```
...
```{r}
ts_df = ts(train_df[["ElevenHr"]], frequency = 288)
plot(ts_df) 
```
```{r}
theme_set(theme_bw())
autoplot(ts_df) + ggtitle("Time Series Plot of the `UoW_electricity_consumption' Time-Series") + 
...
fit = nnetar(ts_df)
nnetforecast <- forecast(fit, h = 400, PI = F)  #Prediction intervals do not come by default in neural net forecasts, in constrast to ARIMA or exponential smoothing model
...
model <- auto.arima(uow_dataset[["11:00"]])
model
```
...
```{r}
# fit neural network
nn=neuralnet(NineHr~TenHr+ElevenHr, data=uow_dataset, hidden=3,act.fct = "logistic",
                linear.output = FALSE)
plot(nn)
```
...
```{r}
kill.pc(winedata_PCA, pc=c(1,9), imputeknn = F, center = T)
```
